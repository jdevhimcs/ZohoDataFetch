...
def get_modules(token):
    url = 'https://www.zohoapis.in/crm/v2.1/settings/modules'
...
def get_module_fields(token, module_name):
    url = f"https://www.zohoapis.in/crm/v2.1/settings/fields?module={module_name}"
    headers = {'Authorization': f'Zoho-oauthtoken {token}'}
    res = requests.get(url, headers=headers)
    res.raise_for_status()
...
def fetch_module_data(token, module_name, from_time=None):
    headers = {'Authorization': f'Zoho-oauthtoken {token}'}
    url = f'https://www.zohoapis.in/crm/v2.1/{module_name}?per_page=200'
